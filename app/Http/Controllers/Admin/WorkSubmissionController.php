...
class WorkSubmissionController extends Controller
{
    ...
    public function update(Request $request, WorkSubmission $submission)
    {
        ...
        // Dispatch event to notify freelancer if status changed to 'needs_revision' or 'approved_by_admin'
        if (in_array($submission->status, ['needs_revision', 'approved_by_admin'])) {
             event(new \App\Events\WorkSubmissionReviewedByAdmin($submission));
        }

        // If 'approved_by_admin', consider updating JobAssignment status to 'completed' or similar
        if ($validated['status'] === 'approved_by_admin') {
    ...
    public function download(WorkSubmission $submission)
    {
        ...
        return Storage::disk('private')->download($submission->file_path, $submission->original_filename);
    }
