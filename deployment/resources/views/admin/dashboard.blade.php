                ...
                @forelse ($recentJobs as $job)
                    <div class="p-3 border border-gray-200 rounded-lg hover:shadow-md transition-shadow bg-gray-50">
                        <div class="flex justify-between items-start">
                            <div>
                                <a href="{{ route('admin.jobs.show', $job) }}" class="font-medium text-cyan-700 hover:text-cyan-600 hover:underline">{{ $job->title }}</a>
                                <p class="text-sm text-gray-600 capitalize">Status: <span class="font-medium">{{ str_replace('_', ' ', $job->status) }}</span></p>
                            </div>
                            <p class="text-xs text-gray-500 whitespace-nowrap ml-2">{{ $job->created_at->format('m/d/Y') }}</p>
                        </div>
                        {{-- You can add more job details here if needed, e.g., client name --}}
                    </div>
                @empty
