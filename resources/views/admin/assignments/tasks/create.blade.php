<x-admin-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Add New Task to Assignment for Job: ') }} {{ $jobAssignment->job->title }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <form method="POST" action="{{ route('admin.job-assignments.tasks.store', $jobAssignment) }}">
                        @csrf
                        ...
                        <div class="flex items-center justify-end mt-6">
                            <a href="{{ route('admin.job-assignments.show', $jobAssignment) }}" class="text-sm text-gray-600 hover:text-gray-900 underline mr-4">
                                {{ __('Cancel') }}
                            </a>
                            <x-primary-button>
                                {{ __('Create Task') }}
